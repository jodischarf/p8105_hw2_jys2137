...
The `snp_df` data includes **`r nrow(snp_df)`** observations and **`r ncol(snp_df)`** variables, related to Standard & Poor’s stock market index (S&P). This included the `year` and `month` of observation, and the associated `close` values of the S&P.
...
The resulting dataset of **`r nrow(fte_join_data)`** observations and **`r ncol(fte_join_data)`** variables shows that between `r min(fte_join_data$year)` and `r max(fte_join_data$year)`, when a republican was `president`, the unemployment rate was **`r filter(fte_join_data, president == "gop") %>% pull(unemployment) %>% mean() %>% round(2)`%** and S&P was


. When a democrat was `president`, the `unemployment` rate was **`r fte_join_data %>% drop_na(unemployment) %>% filter(president == "dem") %>% pull(unemployment) %>% mean() %>% round(2)`%** and the S%P was 
